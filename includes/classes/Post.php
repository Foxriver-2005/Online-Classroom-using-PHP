		...
		public function submitPost($body, $fileName, $fileDestination, $user_to)
{
    ...
    if ($check_empty != "" && $fileName == "") {
        ...
        // If user is on own classroom, user_to is 'none'
        if ($added_by == $user_to) {
            $user_to = 'none';
        }

        // Insert post, exclude id which is auto-incremented
        $query = mysqli_query($this->con, "INSERT INTO posts (body, added_by, courseCode, user_to, date_added, user_closed, edited, files, fileDestination) VALUES ('$check_empty', '$added_by', '$this->code', '$user_to', '$date_added', 'no', 'no', '', '')");
    ...
    if ($fileName != "") { // only assignment
        ...
        // Insert post, exclude id which is auto-incremented
        $query = mysqli_query($this->con, "INSERT INTO posts (body, added_by, courseCode, user_to, date_added, user_closed, edited, files, fileDestination) VALUES ('$body', '$added_by', '$this->code', '$user_to', '$date_added', 'no', 'no', '$fileName', '$fileDestination')");
        $returned_id = mysqli_insert_id($this->con);
